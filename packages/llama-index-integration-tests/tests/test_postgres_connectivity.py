...
@pytest.mark.docker
def test_postgres_table_operations(postgres_container: PostgresContainer) -> None:
    ...
    with psycopg.connect(connection_url) as conn:
        with conn.cursor() as cur:
            ...
            # Insert test data
            cur.execute(
                "INSERT INTO test_connectivity (name, value) VALUES (%s, %s) RETURNING id",
                ("test_entry", 42),
            )
            ...
            # Read back the data
            cur.execute(
                "SELECT name, value FROM test_connectivity WHERE id = %s",
                (inserted_id[0],),
            )
            row = cur.fetchone()
            assert row is not None
            assert row[0] == "test_entry"
            assert row[1] == 42

            # Clean up
            cur.execute("DROP TABLE test_connectivity")
            conn.commit()
