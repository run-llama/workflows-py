...
def _is_dbos(runtime: Runtime) -> bool:
    return type(runtime).__name__ == "DBOSRuntime"


# -- Shared event types (also in tests/conftest.py, redefined here) --
...
async def test_workflow_timeout(runtime: Runtime) -> None:
    if _is_dbos(runtime):
        pytest.xfail(
            "DBOS runtime does not propagate WorkflowTimeoutError the same way"
        )
...
async def test_workflow_event_propagation(runtime: Runtime) -> None:
    ...
    wf = EventTrackingWorkflow(runtime=runtime)
    runtime.launch()
    await WorkflowTestRunner(wf).run()
...
@pytest.mark.asyncio
async def test_workflow_continue_context(runtime: Runtime) -> None:
    if _is_dbos(runtime):
        pytest.xfail("DBOS runtime state store not available for context continuation")
...
async def test_workflow_step_send_event(runtime: Runtime) -> None:
    if _is_dbos(runtime):
        pytest.xfail("DBOS runtime hangs when sending events to named steps")
...
async def test_workflow_num_workers(runtime: Runtime) -> None:
    if _is_dbos(runtime):
        pytest.xfail("DBOS runtime state store not available from adapter")
...
async def test_streaming_e2e(runtime: Runtime) -> None:
    if _is_dbos(runtime):
        pytest.xfail("DBOS stream writes hit UNIQUE constraint on replay")
