...
"""Load-and-replay of pre-child golden fixtures.

These fixtures were captured from pre-child ``origin/main`` (see
``fixtures/prechild_golden/README.md``), so they have the exact wire shape old
readers/writers produced — no ``child_brokers``, no ``namespace_started``, no
per-row ``invocation_namespace``. The recursive-broker refactor must load and
replay them unchanged: this is the hard compat bar (load-compat + round-trip
...
def test_prechild_journal_replays_from_canonical_state() -> None:
    """Replaying the pre-child tick journal from a fresh ``from_workflow`` state
...
async def test_prechild_snapshot_loads_and_resumes() -> None:
    """A pre-child mid-run snapshot (no child fields, waiter without an
    invocation namespace) loads and resumes to completion."""
