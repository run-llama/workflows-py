...
class Context(Generic[MODEL_T]):
    ...
    def _create_internal(
        ...
        Used for testing and scenarios where a Context needs to be
        created already in internal state without going through
        the normal Pre -> External -> Internal flow.
    ...
    def _workflow_run(
        ...
        with instrument_tags({**active_instrument_tags.get(), "run_id": run_id}):
            ...
            external_adapter = workflow._runtime.run_workflow(
                ...
                serialized_state=pre.init_snapshot.state,
    ...
    def store(self) -> InMemoryStateStore[MODEL_T]:
        ...
        """
        return self._require_internal(fn="store").store
