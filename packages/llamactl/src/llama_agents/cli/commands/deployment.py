...
def template_deployment() -> None:
    ...
    display_name = ctx.display_name or Path.cwd().name or None
    ...
    spec = DeploymentSpec(
        ...
        repo_url="" if ctx.is_git_repo else None,
        deployment_file_path=ctx.deployment_file_path or None,
        git_ref=ctx.git_ref if ctx.is_git_repo else None,
        appserver_version=ctx.installed_appserver_version,
        secrets=secrets,
    )
    display = DeploymentDisplay(name=display_name or "my-app", spec=spec)

    head: list[str] = ["Edit, then run: llamactl deployments apply -f <file>"]
    if not ctx.is_git_repo:
        head.append("NOTE: not in a git repo. Add repo_url, or cd into a working tree.")

    field_overrides: dict[str, str] = {}
    if ctx.is_git_repo:
        field_overrides["repo_url"] = (
            "Empty = push mode (CLI pushes your working tree on apply)."
        )

    secret_comments: dict[str, str] = {}
    for name in ctx.required_secret_names:
        if name in ctx.available_secrets:
            secret_comments[name] = "from your .env"
