...
def list_tags(repo: Path, tag_glob: str) -> list[str]:
    """Return tags that match the provided glob sorted newest first."""
    result = subprocess.run(
        ["git", "-C", str(repo), "tag", "-l", tag_glob, "--sort=-version:refname"],
        capture_output=True,
        text=True,
        check=True,
    )
...
def find_previous_tag(repo: Path, tag_prefix: str, current_tag: str) -> str:
