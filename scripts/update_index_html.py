...
def update_index_html(js_url: str, css_url: str, index_path: Path | None = None) -> None:
    ...
    if index_path is None:
        index_path = Path(__file__).parent.parent / "src" / "workflows" / "server" / "static" / "index.html"
    ...
    # Update script src
    script_pattern = r'<script\s+type="module"\s+crossorigin\s+src="[^"]*"[^>]*></script>'
...
def main() -> None:
    ...
    parser.add_argument(
        "--js-url",
        required=True,
        help="URL for the JavaScript file"
    )
    parser.add_argument(
        "--css-url",
        required=True,
        help="URL for the CSS file"
    )
    parser.add_argument(
        "--index-path",
        type=Path,
