            ...
            if enable_snapshots and snapshot_dir is not None:
                try:
                    ...
                    if prev_snapshot is not None:
                        diff = snap.compare_to(prev_snapshot, "lineno")[:tracemalloc_top_n]
...
async def _probe_and_report_workflow_leaks(
    *,
    ...
    if not alive:
        ...
        return
    print(f"[leak-probe] Detected {len(alive)} still-alive workflow instances after run.")
    ...
    def try_objgraph_dump(i: int, obj: object) -> None:
        ...
        try:
            ...
            print("[leak-probe] objgraph not installed; skip PNG backrefs. pip install objgraph graphviz")
            return
        objgraph_dir.mkdir(parents=True, exist_ok=True)
        png_path = objgraph_dir / f"workflow_backrefs_{i}.png"
        try:
            objgraph.show_backrefs([obj], max_depth=ref_chain_depth, filename=str(png_path))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--concurrency", type=int, default=100, help="Max number of concurrent workflow runs"
    )
    parser.add_argument(
        "--sample-interval", type=float, default=0.5, help="Memory sample interval in seconds"
    )
    parser.add_argument(
        "--output",
        type=Path,
