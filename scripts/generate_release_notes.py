...
class GitHubClient:
    ...
    def request_json(self, url: str) -> tuple[object, Mapping[str, str]]:
        ...
        try:
            ...
            raise RuntimeError(
                f"GitHub API call failed ({exc.code} {exc.reason}): {detail}"
            ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate release notes for a package."
    )
    parser.add_argument(
        "--repository", required=True, help="GitHub repository in OWNER/REPO format."
    )
    parser.add_argument(
        "--github-token", required=True, help="GitHub token with repo scope."
    )
    parser.add_argument(
        "--package-label", required=True, help="Label used to select pull requests."
    )
    parser.add_argument(
        "--package-name", required=True, help="Human readable package name."
    )
    parser.add_argument("--current-tag", required=True, help="Current release tag.")
    parser.add_argument(
        "--previous-tag", default="", help="Previous release tag (if available)."
    )
    parser.add_argument(
        "--semver", required=True, help="Semantic version being released."
    )
    parser.add_argument(
        "--output", required=True, help="Path to the GitHub Actions output file."
    )
...
def collect_commit_shas(
    ...
    try:
        ...
        print(
            f"Warning: unable to compare {previous_tag}...{current_tag}: {exc}",
            file=sys.stderr,
        )
        return None
