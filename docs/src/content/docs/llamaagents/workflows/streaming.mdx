...
- **`WorkflowFailedEvent`** - Published when a step fails permanently after exhausting retries. Contains `step_name`, `exception` (the live `Exception`; `__traceback__` is present in-process, `None` after a replay), `attempts`, and `elapsed_seconds`.
