...
### `WorkflowClient(base_url=...) / WorkflowClient(httpx_client=...)`

Create a client. Provide exactly one of `base_url` or `httpx_client`.

### `await client.is_healthy() -> HealthResponse`

Check server health. Raises on failure.

### `await client.list_workflows() -> WorkflowsListResponse`

List registered workflow names.

### `await client.run_workflow(workflow_name, *, start_event=None, context=None, handler_id=None) -> HandlerData`

Run a workflow synchronously (blocks until completion). Returns handler data including the result.

### `await client.run_workflow_nowait(workflow_name, *, start_event=None, context=None, handler_id=None) -> HandlerData`

Start a workflow asynchronously. Returns handler data with a `handler_id` for tracking.

### `client.get_workflow_events(handler_id, *, include_internal_events=False, after_sequence=-1, max_reconnect_attempts=3) -> EventStream`

Returns an `EventStream` — an async iterator of `EventEnvelopeWithMetadata` that also exposes a `last_sequence` property tracking the current stream position. See [Cursor Behavior](#cursor-behavior-after_sequence) for details on `after_sequence`.

### `await client.send_event(handler_id, event, step=None) -> SendEventResponse`

Send an event to a running workflow. Optionally target a specific step.

### `await client.get_handler(handler_id) -> HandlerData`

Get handler metadata and result for a workflow run.

### `await client.get_handlers(*, status=None, workflow_name=None) -> HandlersListResponse`

List all handlers, optionally filtered by status or workflow name.

### `await client.cancel_handler(handler_id, purge=False) -> CancelHandlerResponse`

Cancel a running workflow. Set `purge=True` to also remove it from the persistence store.
