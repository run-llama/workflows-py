...
LlamaAgents is built on top of LlamaIndex Workflows. Use the `llamactl` command line interface (CLI) to scaffold projects (`llamactl init`), run them locally (`llamactl serve`), and deploy to LlamaCloud (`llamactl deployments create`).
