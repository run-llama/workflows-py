...
| controlPlane.container.livenessProbe | object | `{httpGet: {path: /livez, port: http}, periodSeconds: 15, timeoutSeconds: 8, failureThreshold: 4}` | Liveness probe configuration. Checks `/livez` (same kube-apiserver round-trip as `/readyz`) to restart a pod whose client pool is dead and won't self-recover; failureThreshold gives ~60s of sustained failure so a brief apiserver blip doesn't trigger a cascading restart across every replica. |
