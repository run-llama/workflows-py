...
def test_add_workflow(simple_test_workflow):
    server = WorkflowServer()
...
async def test_serve(mock_config, mock_server):
    server = WorkflowServer()
...
@patch("workflows.server.server.uvicorn.Server")
@patch("workflows.server.server.uvicorn.Config")
async def test_serve_with_uvicorn_config(mock_config, mock_server):
    server = WorkflowServer()
    mock_server_instance = AsyncMock()
    mock_server.return_value = mock_server_instance
...
def test_extract_workflow_success(simple_test_workflow):
    server = WorkflowServer()
    server.add_workflow("test", simple_test_workflow)
