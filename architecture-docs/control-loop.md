...
- **Ascent (failure/timeout)** — a child broker whose step fails uncaught locally (no handler, or budget spent), or whose deadline fires uncaught, surfaces to its parent as a boundary *failure* (`_ascend_boundary_failure`): the parent pops the child and routes a `StepFailedEvent` **named for the child's path** through its own wildcard `@catch_error` table with normal `max_recoveries` accounting. Caught → the boundary work item lives on as the handler invocation (still-live, no stream adjustment). Uncaught → the parent itself becomes the failing boundary to *its* parent, recursing; uncaught at the root fails the run. `CommandHalt` stays reserved for root cancel / root timeout.
